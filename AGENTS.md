...
- For library/MCP changes, select the relevant scripts from `package.json` and `.github/workflows/ci.yml` (`npm test`, `npm run test:agent`, `npm run test:mcp:security`, and other scoped suites).
- For Worker work, read `workers/remote-mcp/README.md`; `npm --prefix workers/remote-mcp run check -- --env staging` checks the bundle. The documented staging/production Wrangler route is separate from GitHub Pages.
- The usage guide below also serves consumer agents. Its payment, identity, evidence and free-alternative rules remain binding; ordinary repository development does not authorize buying or collecting posts. Instructions-only changes require no Worker deployment. Observe any existing Pages publication after merge.
